...
Telegram bot developed in **Java + Spring Boot**, which sends motivational quotes to users and provides interactive
features.
...
- The time zone used for scheduling is specified by the property (
  `motivational.telegram.bot.configuration.phrase.time-zone`).
...
- In addition to setting the profile via `SPRING_PROFILES_ACTIVE=prod`, you must specify Redis and database credentials
...
- Make sure the database credentials are set correctly via environment variables or in the
...
When the container needs to access services on the host, replace `localhost` with `host.docker.internal` wherever
necessary
...
- The `UpdateDispatcher` is defined in [
